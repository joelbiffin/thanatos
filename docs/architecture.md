...
A `Prism::Visitor`. **This is where Ruby's semantics are encoded.** It walks the AST and records facts into the `Index`; it makes **no deadness decisions**. It carries six parallel stacks pushed/popped as it enters and leaves scopes: `@scope` (FQN), `@visibility` (ambient `public/protected/private`), `@facts` (current `ClassFacts`), `@method` (enclosing method name, for call attribution), `@singleton_context` (are we in a class-method body?), `@singleton_class` (are we inside `class << self`?).
...
|---|---|---|---|
| 1 | **Visitor pattern** (GoF) + **symbol-table / scope construction** (the standard semantic-analysis pass) | `IndexBuilder`, `LocalVariables` | Double-dispatch AST walk (`Prism::Visitor`): override `visit_X_node`, call `super` to descend. Scope-opening nodes push frames (`enter`/`push_scope`) and pop them on exit (`leave`) — the textbook "stack of scopes" symbol-table build over a DFS. `visit_if_node` **prunes** statically-dead branches before descending. |
